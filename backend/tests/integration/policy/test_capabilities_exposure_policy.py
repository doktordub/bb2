...
def test_capabilities_exposure_policy_hides_policy_internals(
    ...
    with TestClient(app) as client:
        response = client.get("/capabilities", headers={"x-trace-id": "trace-capabilities-policy-0001"})

    assert response.status_code == 200
    payload = response.json()["data"]
    assert "policy" not in payload
    assert "policy_profiles" not in payload["debug"]
    assert "provider" in payload["memory"]