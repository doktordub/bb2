...
def test_stream_payload_policy_keeps_public_sse_contract(
    ...
    with TestClient(app) as client:
        response = client.post(
            "/api/v1/chat/stream",
            headers={"x-trace-id": "trace-stream-policy-int-0001"},
    ...
    assert all("provider_chunk" not in payload for payload in payloads)