...
@pytest.mark.asyncio
async def test_router_runtime_delegates_to_tool_strategy() -> None:
    config = build_config()
    ...
    runtime = DefaultOrchestrationRuntime.from_config(
        config=config,
        llm_gateway=FakeLLMGateway(response_text="router runtime answer"),
    ...
    session_id = "session_router_runtime"
    request = OrchestrationRequest(
        session_id=session_id,
        trace_id="trace_router_runtime",
        user_id="user_1",
    ...
    context = OrchestrationRuntimeContext(
        request_id="request_router_runtime",
        trace_id="trace_router_runtime",
        session_id=session_id,
        user_id="user_1",
    ...
    assert result.answer == "router runtime answer"
