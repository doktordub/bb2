...
def test_reset_clears_workflow_state_only_for_session_integration_slice(
    ...
    with TestClient(app) as client:
        chat = client.post(
            "/chat",
            headers={"x-trace-id": "trace-session-reset-chat-0001"},
            json={"message": "reset me", "session_id": "session_reset_456"},
        )
        reset = client.post(
            "/sessions/session_reset_456/reset",
            headers={"x-trace-id": "trace-session-reset-op-0001"},
    ...
    with sqlite3.connect(trace_database_path) as connection:
        trace_count = connection.execute(
            ...
            ("trace-session-reset-chat-0001", "trace-session-reset-op-0001"),
        ).fetchone()[0]
