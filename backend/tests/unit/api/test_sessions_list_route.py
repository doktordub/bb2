...
def test_list_sessions_route_returns_bounded_session_summaries(
    ...
    with TestClient(app) as client:
        ...
        client.post("/chat", json={"message": "hi", "session_id": "session_456"})

        response = client.get("/sessions?limit=1", headers={"x-trace-id": "trace-list-1234"})

    assert response.status_code == 200
    assert response.headers["x-trace-id"] == "trace-list-1234"
    assert response.json() == {
        "schema_version": "1.0",
        "trace_id": "trace-list-1234",
        "data": {
            "sessions": [
                {
    ...
    assert service.invocations[-1].kind == "list_sessions"


def test_list_sessions_route_rejects_limit_above_configured_max(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path,
) -> None:
    app = build_app(monkeypatch, tmp_path)
    ...
    with TestClient(app) as client:
        response = client.get("/sessions?limit=11")
