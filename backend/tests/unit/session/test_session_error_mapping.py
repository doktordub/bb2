...
def test_chat_route_maps_session_errors_to_stable_api_responses(
    ...
    with TestClient(app) as client:
        app.state.container = replace(
            app.state.container,
            session_service=RaisingSessionService(error),
        )
...
def test_reset_route_maps_session_reset_failed_to_stable_api_response(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path,
) -> None:
    app = build_app(monkeypatch, tmp_path)
    ...
    with TestClient(app) as client:
        app.state.container = replace(
            app.state.container,
            session_service=RaisingSessionService(SessionResetFailedError()),
        )
        response = client.post("/sessions/session_123/reset", json={})
    ...
    assert response.json() == {
        ...
        "error": {
            ...
            "message": "The session could not be reset.",
            "retryable": True,
            "details": {},
        },
    }