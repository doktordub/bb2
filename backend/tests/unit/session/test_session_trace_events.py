...
def _build_context():
    return build_session_request_context(
        trace_id="trace-session-trace-0001",
        request_id="request-session-trace-0001",
        user_id="local_user",
        ...
        headers_safe={"x-trace-id": "trace-session-trace-0001"},
    )


@pytest.mark.asyncio
async def test_session_trace_event_is_safe_and_does_not_expose_message_text() -> None:
    ...
    assert "secret user prompt" not in str(event.payload)
    assert event.agent_name == "fake_session_agent"
    assert event.strategy_name == "fake_direct_strategy"
    assert event.llm_profile == "fake_local_profile"


@pytest.mark.asyncio
async def test_session_trace_event_uses_resumed_name_for_existing_session() -> None:
    workflow_state = FakeWorkflowStateStore()
    workflow_state.states["session_trace_2"] = {
        "conversation": {"messages": [{"role": "user", "content": "hello"}]},
        "workflow": {"current_step": "answered"},
        "metadata": {"loaded_empty": False},
    }
    workflow_state.versions["session_trace_2"] = 2
    trace_store = FakeTraceStore()
    service = DefaultSessionService(
        config=FakeConfigurationView({"usecases": {"default_chat": {"enabled": True}}}),
        settings=_build_settings(),
        ...
        clock=FakeClock(
            [
                datetime(2026, 6, 27, 14, 30, tzinfo=UTC),
                datetime(2026, 6, 27, 14, 30, 1, tzinfo=UTC),
            ]
        ),
    )
    ...
    assert len(trace_store.events) == 1
    assert trace_store.events[0].resolved_event_name == "session_resumed"
